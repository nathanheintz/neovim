...
return {
	...
	opts = {
		setup = {
			...
			icons = {
				...
				colors = false, -- Disable hardcoded colors, inherit from colorscheme
			},
			layout = {
		...
		defaults = {
			...
			e = { "<cmd>Neotree toggle<CR>", "explorer", icon = "" },
			q = { "<cmd>wa! | qa!<CR>", "quit" },
			u = { "<cmd>Telescope undo<CR>", "undo", icon = "" },
			w = {
				name = "WINDOW",
			...
			f = {
				name = "FIND",
				a = {
					"<cmd>lua require('telescope.builtin').find_files({ no_ignore = true, hidden = true, search_dirs = { '~/' } })<CR>",
					"all files",
				},
			...
			m = {
				...
				t = {
					name = "TOGGLES",
					b = { "<cmd>lua _G.toggle_buffer_completion()<CR>", "toggle buffer completion", icon = " " },
	...
	config = function(_, opts)
		...
		local logfile = vim.fn.expand("~/.config/nvim/whichkey-debug.log")
		vim.api.nvim_create_autocmd("ModeChanged", {
			callback = function()
				local f = io.open(logfile, "a")
				if f then
					f:write(string.format("[%s] Mode changed: %s\n", os.date("%H:%M:%S"), vim.fn.mode()))
					f:close()
				end
			end,
		})

		-- Log every key press using vim.on_key
		vim.on_key(function(key, typed)
			local f = io.open(logfile, "a")
			if f then
				local readable_key = vim.fn.keytrans(key)
				local readable_typed = vim.fn.keytrans(typed)
				f:write(string.format("[%s] Key: %s (typed: %s) Buffer: %d Mode: %s\n",
					os.date("%H:%M:%S"), readable_key, readable_typed, vim.api.nvim_get_current_buf(), vim.fn.mode()))
				f:close()
			end
		end)
	end,
